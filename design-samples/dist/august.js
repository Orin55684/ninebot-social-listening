...
async function refreshData(){
 ...
 }catch(e){$('#clock-error').textContent=e.message}
 finally{refreshing=false;const b=$('#refresh-data');b.disabled=false;b.textContent='↻ 刷新数据'}
}
async function changeClock(change){
 try{await api('api/clock',change);riskPage=1;workspace.report=null;await refreshData()}catch(e){$('#clock-error').textContent=e.message}
}
